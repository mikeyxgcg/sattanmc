  ...
  client.on('message', async message => {
    ...
    .setImage('https://c.top4top.net/p_1029o1te41.png')
      ...
      if(message.content.startsWith(prefix + "mute")) {
          ...
          if(message.guild.member(mutePerson).roles.has(muteRole.id)) return message.channel.send('**-هذا الاعب ميوت مسبقا**');
          if(!muteRole) return message.guild.createRole({ name: "Muted", permissions: [] });
          if(!time) return message.channel.send("**-الوقت**");
          ...
          if(!muteReason) return muteReason = "**السبب **";
...
client.on('message',message =>{
  var command = message.content.toLowerCase().split(" ")[0];
    ...
    if(command == prefix + 'unban') {
        ...
        if(!args[1]) return  message.channel.send(':information_source:  `-unban <@id>`You must select someone');
        if(args[1].length < 16) return message.reply(':no_entry: | This ID is not id user!');
        message.guild.fetchBans().then(bans => {
            var Found = bans.find(m => m.id === args[1]);
            if(!Found) return message.channel.send(`:no_entry: | <@${message.author.id}> هذا شخص ليس متبند في سيرفر! :unlock:`);
...
3- :warning: صيانة يومية
4- :money_with_wings: مجاني بالكامل
5- :shield: يحتوي علي مانع اختراق السيرفرات
...
-server : لمعرفة معلومات سيرفر

-move : لسحب العضو الي روم صوتي حقك

-clear : لمسح الشات مع اختيار عدد مسح رسائل

-avatar :يظهر لك صورة حقتك

-image : يوريك صورة سيرفر

-credit : لمعرفة كم كريديت معاك

-daily : عشان تاخذ فلوس يومية

-rep : تاخذ ريب 

-rank : يوريك رانكات

-profile : يوريك البروفايل حقك

اوامر ادارية

-ban : لاعطاء بان للعضو

-setwelcomer : عشان تحط مكان الولكم الي بيسوي فيه البوت

-autorole : يعطي اي واحد يدخل رتبة الي تبيها

-kick : يطرد لاعب من سيرفر

-mute : لاعطاء ميوت لشخص

-unmute : لفك الميوت من شخص

-warn : لتحذير او انذار الشخص
...
if(message.content.startsWith(prefix  +  'warn'))  {
  ...
  if(!user)  return  message.channel.send("**  -  منشن الاعب  **")//by  OrochiX
  if(!reason)  return  message.channel.send("**  -  اكتب السبب **")//by  OrochiX
...
if(!incidentchannel) return message.channel.send("ليست موجودةروم warn");
                       ...
                       client.on('message' , message => {
                        if (message.content.startsWith(prefix + "time")) {
                         if(!message.member.hasPermission(`MANAGE_GUILD`)) return;
                          ...
                          if(!newTime) return message.reply(`**${prefix}time <time>  \`1000 = 1s\`**`)
...
if(message.content.startsWith(prefix + 'credis ')) {
          if (!args[0]) {
            message.channel.send(`**Usage: ${prefix}credit 
aaaaaaaa			@someone amount**`);
         return;
           }
...
${videos.map(video2 => `[**${++index}**] **${video2.title}**`).join('\n')}`)
    ...
    return msg.channel.send(`:speaker: ت5/م تغير الصوت الي **${args[1]}**`);
  } else if (command === `-np`) {
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
  ...
  if (!serverQueue) {
    const queueConstruct = {
      ...
      /* voiceChanael: voiceChannel, */
      connection: nuaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
      volume: 5,
      playing: true
    };
...
client.on('message', async message => {
    ...
    let args = message.content.split(' ').slice(1);
if (message.content.startsWith("-     edeletecolors")) {
  if(!message.member.hasPermission('ADMINISTRATOR')) return
...
client.on('message', msg => {
 if (msg.content.startsWith(prefix + 'حساب')) {
    let args = msg.content.split(" ").slice(1);
